...
class WebServer(object):
  ...
  def generate_token(self, length=12) -> str:
    ...
    while True:
      ...
      if not random_token in self.registered_tokens:
        return random_token
  ...
  def has_valid_token(self, request:object) -> bool:
    ...
    if not 'session_token' in dict(request.cookies):
      return False
    # if the session_token is not valid
    if not request.cookies['session_token'] in list(self.registered_tokens.keys()):
      return False
    # checks passed
    return True
